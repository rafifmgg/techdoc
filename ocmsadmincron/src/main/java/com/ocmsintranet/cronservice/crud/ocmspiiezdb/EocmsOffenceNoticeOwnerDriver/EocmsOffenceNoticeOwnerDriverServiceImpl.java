...
public class EocmsOffenceNoticeOwnerDriverServiceImpl
    ...
    @Override
    @Transactional(value = "piiTransactionManager", readOnly = true)
    public Optional<EocmsOffenceNoticeOwnerDriver> findByNoticeNoAndOwnerDriverIndicator(String noticeNo, String ownerDriverIndicator) {
        openSymmetricKey();
        try {
            return repository.findByNoticeNoAndOwnerDriverIndicator(noticeNo, ownerDriverIndicator);
        } finally {
            closeSymmetricKey();
        }
    }
