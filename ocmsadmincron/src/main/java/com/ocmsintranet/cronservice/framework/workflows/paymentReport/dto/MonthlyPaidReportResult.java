...
/**
 * Result wrapper for Monthly Paid Report generation operations.
 * Contains execution status and processing statistics for detailed response messages.
 */
@Data
public class MonthlyPaidReportResult {
    ...
    public MonthlyPaidReportResult(boolean success, String message) {
    ...
    public MonthlyPaidReportResult(boolean success, String message, int totalCount,
                                    int eServiceCount, int axsCount, int offlineCount,
                                    int jtcCollectionCount, int refundCount, String reportMonth) {
        this.success = success;
        this.message = message;
    ...
    public String getDetailedMessage() {
        ...
        if (totalCount == 0) {
            return "No paid records found for " + reportMonth + " - report generation skipped";
        }

        return String.format("Processed %d paid records for %s - eService: %d, AXS: %d, Offline: %d, JTC: %d, Refund: %d",
                totalCount, reportMonth, eServiceCount, axsCount, offlineCount, jtcCollectionCount, refundCount);
    }
}
