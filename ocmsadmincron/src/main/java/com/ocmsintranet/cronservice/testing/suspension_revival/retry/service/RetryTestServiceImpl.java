...
import com.ocmsintranet.cronservice.testing.suspension_revival.retry.helpers.RetryStepExecutionHelper;
...
public class RetryTestServiceImpl implements RetryTestService {
    ...
    @Autowired
    private RetryStepExecutionHelper stepExecutionHelper;

    private static final String SCENARIO_FILE = "testing/suspension_revival/data/retry_scenarios.json";

    @Override
    public SuspensionRevivalTestResponse executeRetryTest(SuspensionRevivalTestRequest request) {
        log.info("Executing Retry test: details={}, triggerApi={}",
        ...
        log.info("Retry test completed: {} steps executed", response.getSteps().size());
        return response;
    }
