 ...
 *                  when MHA response shows street="NA" and postal_code="000000"
 *
 * 6-Step Flow:
 * 1. Setup test data with NRIC that will return street="NA" + postal="000000"
 * 2. POST /mha/upload - Trigger upload process
 * 3. Verify files in SFTP - Check generated files exist
 * 4. POST /test/mha/mha-callback - Simulate MHA response with street="NA" + postal="000000"
 * 5. POST /mha/download/execute - Trigger download process
 * 6. Verify TS-NRO suspension applied for specific address pattern
 */
@Slf4j
@Service
public class MhaFutureDateAddressChangeFlowService {
    ...
    // Test data constants - using NRIC that will return street="NA" + postal="000000"
    ...
    public List<TestStepResult> executeFlow() {
        ...
        try {
            ...
            // Step 4: POST /test/mha/mha-callback with street="NA" + postal="000000"
    ...
    /**
     * Step 1: Setup test data with NRIC that will return street="NA" + postal="000000".
     */
    private TestStepResult setupTestData() {
        ...
        try {
            ...
            if (recordCount != null && recordCount > 0) {
                ...
                result.addDetail("🆔 NRIC: " + TEST_NRIC + " (will return street=\"NA\" + postal=\"000000\")");
            } else {
    ...
    /**
     * Step 4: POST /test/mha/mha-callback - Simulate MHA response with street="NA" + postal="000000".
     */
    private TestStepResult simulateMhaCallbackWithSpecificAddress() {
        TestStepResult result = new TestStepResult("Step 4: POST /test/mha/mha-callback - Simulate MHA Response with street=\"NA\" + postal=\"000000\"", STATUS_INFO);

        try {
            log.info("📞 Simulating MHA callback with street=\"NA\" + postal=\"000000\" for: {}", NOTICE_NUMBER);
            ...
            result.addDetail("📍 Address: street=\"" + STREET + "\", postal=\"" + POSTAL_CODE + "\"");
        } catch (Exception e) {
    ...
    private TestStepResult verifyStreetAddressValidationSuspension() {
        ...
        try {
            ...
            verificationResults.add("  🏠 Address pattern detected: street=\"" + STREET + "\", postal=\"" + POSTAL_CODE + "\"");
            verificationResults.add("  📍 Trigger condition: Street = \"NA\" and postal code = \"000000\"");
            ...
            if (allVerificationsPassed) {
                ...
                result.addDetail("📋 Expected behavior: TS-NRO applied when street=\"NA\" + postal=\"000000\"");
    ...
    private void generateMhaResponseFilesWithSpecificAddress(TestStepResult result, String noticeNumber) {
        try {
            ...
            result.addDetail("🏠 Address validation: street=\"" + STREET + "\", postal=\"" + POSTAL_CODE + "\"");
    ...
    private String generateResponseWithSpecificAddress(String noticeNumber) {
        ...
        sb.append(String.format("%-9s", TEST_NRIC)) // ID Number
          ...
          .append(String.format("%-32s", STREET)) // MHA Street Name
          ...
          .append(String.format("%-6s", POSTAL_CODE)) // MHA New Postal Code
          ...
          .append(String.format("%-8s", LocalDateTime.now().plusYears(1).format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss")))) // Date of Address Change
