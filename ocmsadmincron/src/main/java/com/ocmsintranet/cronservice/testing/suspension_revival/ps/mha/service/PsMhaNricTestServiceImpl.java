...
import com.ocmsintranet.cronservice.testing.suspension_revival.ps.mha.helpers.PsMhaNricStepExecutionHelper;
...
@Slf4j
@Service
public class PsMhaNricTestServiceImpl implements PsMhaNricTestService {

    @Autowired
    private PsMhaNricStepExecutionHelper stepExecutionHelper;

    private static final String SCENARIO_FILE = "testing/suspension_revival/data/ps_mha_nric_scenarios.json";

    @Override
    public SuspensionRevivalTestResponse executePsMhaNricTest(SuspensionRevivalTestRequest request) {
        log.info("Executing PS-MHA NRIC test: details={}, triggerApi={}",
        ...
        log.info("PS-MHA NRIC test completed: {} steps executed", response.getSteps().size());
        return response;
    }
