package com.ocmsintranet.cronservice.testing.suspension_revival.ps.toppan.controllers;
...
import com.ocmsintranet.cronservice.testing.suspension_revival.ps.toppan.service.PsToppanTestService;
import com.ocmsintranet.cronservice.testing.suspension_revival.ps.toppan.service.PsToppanTestSetupService;
...
public class PsToppanTestController {
    ...
    @Autowired
    private PsToppanTestService testService;

    @Autowired
    private PsToppanTestSetupService setupService;

    @PostMapping("/ps-toppan-test")
    public ResponseEntity<SuspensionRevivalTestResponse> handlePsToppanTest(
        ...
        log.info("Received POST /v1/ps-toppan-test with details={}, triggerApi={}",
                request.getDetails(), request.getTriggerApi());

        SuspensionRevivalTestResponse response = testService.executePsToppanTest(request);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/ps-toppan-test-setup")
    public ResponseEntity<Map<String, Object>> handleSetup() {
        log.info("Received POST /v1/ps-toppan-test-setup");
    ...
    @DeleteMapping("/ps-toppan-test-cleanup")
    public ResponseEntity<Map<String, Object>> handleCleanup() {
        log.info("Received DELETE /v1/ps-toppan-test-cleanup");
