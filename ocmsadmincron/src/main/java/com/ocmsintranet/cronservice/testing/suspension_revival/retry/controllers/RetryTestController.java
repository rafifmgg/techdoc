...
import com.ocmsintranet.cronservice.testing.suspension_revival.retry.service.RetryTestService;
import com.ocmsintranet.cronservice.testing.suspension_revival.retry.service.RetryTestSetupService;
...
public class RetryTestController {
    ...
    @Autowired
    private RetryTestSetupService setupService;

    @PostMapping("/retry-test")
    public ResponseEntity<SuspensionRevivalTestResponse> handleRetryTest(
        ...
        log.info("Received POST /v1/retry-test with details={}, triggerApi={}",
                request.getDetails(), request.getTriggerApi());

        SuspensionRevivalTestResponse response = testService.executeRetryTest(request);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/retry-test-setup")
    public ResponseEntity<Map<String, Object>> handleSetup() {
        log.info("Received POST /v1/retry-test-setup");
    ...
    @DeleteMapping("/retry-test-cleanup")
    public ResponseEntity<Map<String, Object>> handleCleanup() {
        log.info("Received DELETE /v1/retry-test-cleanup");
