...
/**
 * Service for DataHive Snowflake Contact Test - UEN Not Found Flow
 * Implements simple 4-step test for UEN not found in Snowflake
 */
@Service
public class DataHiveContactUenNotFound {
    ...
    // Test data constants - UEN not found scenario
    private static final String TEST_NOTICE_PREFIX = "TESTSF";
    private static final String NOTICE_NO = TEST_NOTICE_PREFIX + "004";
    private static final String TEST_VEHICLE_NO = "EM8670X";
    private static final String TEST_UEN = "INVALIDUEN123";  // Invalid UEN for testing not found
    ...
    @Autowired
    public DataHiveContactUenNotFound(SftpUtil sftpUtil) {
        this.sftpUtil = sftpUtil;
    }

    /**
     * Run the simplified 4-step test flow for UEN not found in Snowflake
     *
    ...
    public List<TestStepResult> runTest() {
        ...
        // Step 1: Setup test data with ENA->ROV status and invalid UEN
        ...
        // Step 4: Verify no email update and processing stage = RD1
        if (continueExecution) {
     ...
     * Uses invalid UEN for Snowflake not found testing
     *
     * @return TestStepResult containing the result of this step
     */
    private TestStepResult setupTestDataForSnowflake() {
        TestStepResult result = new TestStepResult(
            "Step 1: Setup test data for Snowflake UEN not found testing (ENA->ROV, Invalid UEN)",
            STATUS_SUCCESS
        );
        ...
        try {
            ...
            // Insert owner/driver record with invalid UEN
            insertOwnerDriverWithInvalidUen(result);
    ...
    /**
     * Step 4: Verify no email update and processing stage progressed to RD1
     *
     * @return TestStepResult containing the result of this step
     */
    private TestStepResult verifySnowflakeNotFoundResult() {
        TestStepResult result = new TestStepResult(
            "Step 4: Verify Snowflake UEN not found result",
            STATUS_SUCCESS
        );

        try {
            result.addDetail("🔍 Verifying Snowflake UEN not found processing...");

            // Query owner/driver record to check no email updates
            ...
            // Verify no email updates (expected for not found scenario)
            boolean noEmailUpdates = true;
            if (emailAddr != null && !emailAddr.trim().isEmpty()) {
                result.addDetail("⚠️ Unexpected email address update: " + emailAddr);
                noEmailUpdates = false;
            }

            if (noEmailUpdates) {
                result.addDetail("✅ No email updates as expected for UEN not found");
            }
    ...
    /** Insert owner/driver record with invalid UEN */
    private void insertOwnerDriverWithInvalidUen(TestStepResult result) {
        ...
        jdbcTemplate.update(sql, NOTICE_NO, "INVALID COMPANY NAME PTE LTD", TEST_UEN, TEST_ID_TYPE, "Y", "TEST_USER", LocalDateTime.now());

        result.addDetail("✅ Owner/driver record inserted with invalid UEN: " + TEST_UEN + " (ID type: " + TEST_ID_TYPE + ")");
    }
    ...
    private String generateLtaResponseFileContent() {
        ...
        // Data record - LTA response format with invalid UEN
        content.append("D")
            ...
            .append(String.format("%-20s", TEST_UEN))                  // Owner ID (Invalid UEN)
            .append(String.format("%-66s", "INVALID COMPANY NAME PTE LTD")) // Owner Name
    ...
    private void verifyProcessingStageProgressionToRD1(TestStepResult result) {
        ...
        try {
            ...
            // Expected: ROV->RD1 (skipping ENA because UEN not found)
