...
@Slf4j
@Service
public class PsMhaNricTestSetupService {
    ...
    private static final String SCENARIO_FILE = "testing/suspension_revival/data/ps_mha_nric_scenarios.json";

    public Map<String, Object> setupTestData() {
        log.info("Starting PS-MHA NRIC test data setup");

        int noticesCreated = 0;
        int mhaFilesCreated = 0;
        ...
        try {
            ...
            result.put("mha_files_created", mhaFilesCreated);
            result.put("scenarios_loaded", scenarios.size());

            log.info("PS-MHA NRIC test data setup completed: {} notices, {} MHA files",
                    noticesCreated, mhaFilesCreated);

            return result;

        } catch (Exception e) {
            log.error("PS-MHA NRIC test data setup failed: {}", e.getMessage(), e);
    ...
    public Map<String, Object> cleanupTestData() {
        log.info("Starting PS-MHA NRIC test data cleanup");
        ...
        try {
            ...
            log.info("PS-MHA NRIC test data cleanup completed: {}", deletedCounts);
            return result;

        } catch (Exception e) {
            log.error("PS-MHA NRIC test data cleanup failed: {}", e.getMessage(), e);
            Map<String, Object> errorResult = new HashMap<>();
            errorResult.put("status", "error");
