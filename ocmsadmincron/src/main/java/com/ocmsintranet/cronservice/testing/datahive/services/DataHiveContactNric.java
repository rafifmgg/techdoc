...
/**
 * Service for DataHive Snowflake Contact Test - NRIC Flow
 * Implements simple 4-step test for Snowflake contact lookup functionality
 */
@Service
public class DataHiveContactNric {
    ...
    private static final String NOTICE_NO = TEST_NOTICE_PREFIX + "001";
    private static final String TEST_VEHICLE_NO = "EM8667X";
    private static final String TEST_NRIC = "F6120458W";  // Valid NRIC for testing
    ...
    @Autowired
    public DataHiveContactNric(SftpUtil sftpUtil) {
        this.sftpUtil = sftpUtil;
    }

    /**
     * Run the simplified 4-step test flow for Snowflake contact lookup
     *
    ...
    public List<TestStepResult> runTest() {
        ...
        // Step 1: Setup test data with ENA->ROV status and valid NRIC
        ...
        // Step 4: Verify Snowflake contact data update
        if (continueExecution) {
            TestStepResult step4 = verifySnowflakeContactUpdate();
            results.add(step4);
        }
    ...
    private TestStepResult setupTestDataForSnowflake() {
        ...
        try {
            ...
            insertOwnerDriverWithNric(result);
    ...
    /**
     * Step 4: Verify Snowflake contact data has been updated
     *
     * @return TestStepResult containing the result of this step
     */
    private TestStepResult verifySnowflakeContactUpdate() {
        TestStepResult result = new TestStepResult(
            "Step 4: Verify Snowflake contact data update",
            STATUS_SUCCESS
        );
        ...
        try {
            result.addDetail("🔍 Verifying Snowflake contact data update...");

            // Query owner/driver record to check contact updates
            ...
            // Verify Snowflake processing occurred
            boolean snowflakeProcessed = false;

            // For NRIC (ID type 1), we expect telephone number to be updated
            if (telNo != null && !telNo.trim().isEmpty()) {
                result.addDetail("✅ Telephone number updated by Snowflake: " + telNo);
                snowflakeProcessed = true;
            }

            // Check if DataHive processing metadata is present
            if (snowflakeProcessed) {
                result.addDetail("🎉 Snowflake contact lookup processing verified successfully!");
            } else {
                result.addDetail("⚠️ No evidence of Snowflake processing found");
                result.addDetail("💡 This could indicate the ID is not found in DataHive or processing failed");
            }

            // Verify processing stage progression
            verifyProcessingStageProgression(result);
    ...
    /** Insert owner/driver record with valid NRIC */
    private void insertOwnerDriverWithNric(TestStepResult result) {
        ...
        result.addDetail("✅ Owner/driver record inserted with NRIC: " + TEST_NRIC + " (ID type: " + TEST_ID_TYPE + ")");
    }
    ...
    private String generateLtaResponseFileContent() {
        ...
        // Data record - LTA response format with all required fields
        content.append("D")
            ...
            .append(String.format("%-20s", TEST_NRIC))                 // Owner ID
            ...
            .append("0")                                               // Return Error Code (space = no error)
    ...
    /** Verify processing stage has progressed correctly */
    private void verifyProcessingStageProgression(TestStepResult result) {
        ...
        try {
            ...
            // Expected: Should have progressed from ENA->ROV to either ENA->RD1 or ROV->ENA depending on Snowflake result
            if ("ENA".equals(lastStage) || "ROV".equals(lastStage)) {
                result.addDetail("✅ Processing stage progressed as expected");
            } else {
                result.addDetail("⚠️ Unexpected processing stage progression");
            }
        } catch (Exception e) {
