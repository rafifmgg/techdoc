...
/**
 * Service for testing MHA FIN Processing Flow scenario.
 *
 * Test scenario: Foreign ID number handling
 * Expected result: Owner/Driver record updated with FIN data from MHA response
 *                  id_type="F", id_no updated with FIN
 *
 * 6-Step Flow:
 * 1. Setup test data with notice containing FIN instead of NRIC
 * 2. POST /mha/upload - Trigger upload process
 * 3. Verify files in SFTP - Check generated files exist
 * 4. POST /test/mha/mha-callback - Simulate MHA response with valid FIN data
 * 5. POST /mha/download/execute - Trigger download process
 * 6. Verify FIN processing applied with id_type="F" and updated owner data
 */
@Slf4j
@Service
public class MhaFinProcessingFlowService {
    ...
    // Test data constants for FIN processing scenario
    private static final String SCHEMA = "ocmsizmgr";
    private static final String TEST_VEHICLE_NO = "SBA9012J";
    private static final String TEST_FIN = "G1234567X";  // Foreign ID Number
    private static final String TEST_ID_TYPE = "F";  // Foreign ID type
    private static final String TEST_OWNER_NAME = "Kumar Rajesh";
    private static final String TEST_NATIONALITY = "Indian";
    private static final String TEST_COUNTRY_CODE = "IN";
    ...
    /**
     * Execute complete FIN Processing Flow test with 6 steps.
     *
    ...
    public List<TestStepResult> executeFlow(String noticePrefix) {
        log.info("🚀 Starting MHA FIN Processing Flow Test...");
        ...
        try {
            ...
            // Step 4: POST /test/mha/mha-callback with FIN data
            steps.add(simulateMhaCallbackWithFin(noticePrefix));

            // Step 5: POST /mha/download/execute
            steps.add(triggerMhaDownload(noticePrefix));

            // Step 6: Verify FIN processing applied
            steps.add(verifyFinProcessing(noticePrefix));

        } catch (Exception e) {
            log.error("❌ Critical error during FIN Processing flow execution: {}", e.getMessage(), e);
        ...
        log.info("✅ MHA FIN Processing Flow Test completed. Summary: Success: %d, Failed: %d, Skipped: %d",
    ...
    /**
     * Step 1: Setup test data with notice containing FIN instead of NRIC.
     */
    private TestStepResult setupTestData(String noticePrefix) {
        TestStepResult result = new TestStepResult("Step 1: Setup Test Data - FIN Processing Scenario", STATUS_INFO);

        try {
            String noticeNumber = noticePrefix + "001";
            log.info("🔧 Setting up test data for FIN Processing scenario: {}", noticeNumber);
            ...
            if (recordCount != null && recordCount > 0) {
                ...
                result.addDetail("🆔 FIN: " + TEST_FIN + " (ready for FIN processing)");
                result.addDetail("🌐 ID Type: F (Foreign)");
            } else {
    ...
    /**
     * Step 4: POST /test/mha/mha-callback - Simulate MHA response with FIN data.
     */
    private TestStepResult simulateMhaCallbackWithFin(String noticePrefix) {
        TestStepResult result = new TestStepResult("Step 4: POST /test/mha/mha-callback - Simulate MHA Response with FIN Data", STATUS_INFO);

        try {
            String noticeNumber = noticePrefix + "001";
            log.info("📞 Simulating MHA callback with FIN data for: {}", noticeNumber);

            // Generate MHA response files with FIN information
            ...
            result.addDetail("🆔 FIN data for: " + TEST_FIN + " (" + TEST_NATIONALITY + ")");
    ...
    /**
     * Step 6: Verify FIN processing applied with id_type="F" and updated owner data.
     */
    private TestStepResult verifyFinProcessing(String noticePrefix) {
        TestStepResult result = new TestStepResult("Step 6: Verify FIN Processing Applied", STATUS_INFO);

        try {
            String noticeNumber = noticePrefix + "001";
            log.info("🔍 Verifying FIN processing for: {}", noticeNumber);

            boolean allVerificationsPassed = true;
            List<String> verificationResults = new ArrayList<>();

            // Verify 1: Owner/Driver record updated with FIN data
            ...
            try {
                ...
                if ("F".equals(idType) && TEST_FIN.equals(idNo)) {
                    verificationResults.add("✅ FIN Processing Applied Successfully:");
                    verificationResults.add("  🆔 ID Type: " + idType + " (Foreign)");
                    verificationResults.add("  🔢 FIN: " + idNo);
                    ...
                    verificationResults.add("❌ FIN Processing Failed:");
                    verificationResults.add("  Expected: id_type=F, id_no=" + TEST_FIN);
                    verificationResults.add("  Got: id_type=" + idType + ", id_no=" + idNo);
                    allVerificationsPassed = false;
                }

            } catch (Exception e) {
                verificationResults.add("❌ FIN processing verification failed: " + e.getMessage());
                allVerificationsPassed = false;
            }
            ...
            if (suspensionCount != null && suspensionCount == 0) {
                ...
                verificationResults.add("⚠️ Suspension Check: Found " + suspensionCount + " suspension records (unexpected for FIN processing)");
            }
            ...
            // Verify 4: FIN format validation
            if (TEST_FIN.matches("^[G][0-9]{7}[A-Z]$")) {
                verificationResults.add("✅ FIN Format Validation: Valid FIN pattern (G#######L)");
            } else {
                verificationResults.add("⚠️ FIN Format Validation: FIN pattern may need review: " + TEST_FIN);
            }

            // Final assessment
            if (allVerificationsPassed) {
                result.setStatus(STATUS_SUCCESS);
                result.addDetail("✅ FIN Processing verification completed successfully");
                result.addDetail("📋 Expected behavior: Owner/Driver updated with FIN data (id_type=F)");
                result.addDetail("🎯 Scenario outcome: PASSED - FIN Processing flow working correctly");
            } else {
                result.setStatus(STATUS_FAILED);
                result.addDetail("❌ FIN Processing verification failed");
            ...
            log.error("❌ Error during FIN Processing verification: {}", e.getMessage(), e);
            result.setStatus(STATUS_FAILED);
    ...
    private void insertOffenceNoticeDetail(TestStepResult result, String noticeNo) {
        ...
        jdbcTemplate.update(sql, noticeNo, "MHTEST567890123", "TOYOTA", "RED",
    ...
    private void insertOwnerDriver(TestStepResult result, String noticeNo) {
        String sql = "INSERT INTO " + SCHEMA + ".ocms_offence_notice_owner_driver (" +
                     ...
                     "nationality, cre_user_id, cre_date) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        jdbcTemplate.update(sql, noticeNo, "O", TEST_ID_TYPE, TEST_FIN, TEST_OWNER_NAME, "Y",
            TEST_NATIONALITY, "TEST_USER", LocalDateTime.now());

        result.addDetail("✅ Owner/driver record inserted: " + noticeNo + " (FIN: " + TEST_FIN + ")");
        result.addDetail("   - ID Type: F (Foreign)");
    ...
    private void generateMhaResponseFiles(TestStepResult result, String noticeNumber) {
        try {
            // Generate NRO2URA file with FIN information
            String mainContent = generateMainFileWithFin();
            ...
            result.addDetail("📤 Generated NRO2URA file with FIN data: " + mainFileName);
            result.addDetail("📋 FIN content: " + TEST_FIN + " with nationality " + TEST_NATIONALITY);
    ...
    private String generateMainFileWithFin() {
        // Generate NRO2URA file content with FIN information
