...
@Slf4j
@Service
public class TsDatahiveBatchTestSetupService {
    ...
    private static final String SCENARIO_FILE = "testing/suspension_revival/data/ts_datahive_batch_scenarios.json";

    public Map<String, Object> setupTestData() {
        log.info("Starting TS-DataHive Batch test data setup");

        int noticesCreated = 0;
        int datahiveBatchFilesCreated = 0;
        ...
        try {
            ...
            result.put("datahive_batch_files_created", datahiveBatchFilesCreated);
            result.put("scenarios_loaded", scenarios.size());

            log.info("TS-DataHive Batch test data setup completed: {} notices, {} DataHive batch files",
                    noticesCreated, datahiveBatchFilesCreated);

            return result;

        } catch (Exception e) {
            log.error("TS-DataHive Batch test data setup failed: {}", e.getMessage(), e);
    ...
    public Map<String, Object> cleanupTestData() {
        log.info("Starting TS-DataHive Batch test data cleanup");
        ...
        try {
            ...
            log.info("TS-DataHive Batch test data cleanup completed: {}", deletedCounts);
            return result;

        } catch (Exception e) {
            log.error("TS-DataHive Batch test data cleanup failed: {}", e.getMessage(), e);
            Map<String, Object> errorResult = new HashMap<>();
            errorResult.put("status", "error");
