...
/**
 * Service for LTA Local Normal Flow test scenario
 * Implements Test Flow 1: Local Normal Flow
 */
@Service
public class LtaLocalNormalFlowService {
    ...
    private static final String NOTICE_NO = TEST_NOTICE_PREFIX + "LOC001";
    ...
    @Autowired
    public LtaLocalNormalFlowService(SftpUtil sftpUtil, AzureBlobStorageUtil azureBlobStorageUtil) {
        this.sftpUtil = sftpUtil;
        this.azureBlobStorageUtil = azureBlobStorageUtil;
    }

    /**
     * Run the test flow for Local Normal Flow scenario
     *
    ...
    /**
     * Step 1: Initialize test data for Local Normal Flow
    ...
    private void insertValidOffenceNotice(TestStepResult result) {
        ...
        int inserted = jdbcTemplate.update(sql,
            ...
            "EM8667X",                          // vehicle_no (String, length = 12)
            "S",                                // vehicle_registration_type (String, length = 1) - S for local
    ...
    private void verifyDataInserted(TestStepResult result) {
        ...
        if (!"S".equals(vehicleRegType)) {
            result.setStatus(STATUS_FAILED);
            result.addDetail("❌ vehicle_registration_type is not 'S' (Local): " + vehicleRegType);
        }

        if ("N".equals(diplomaticFlag)) {
            result.setStatus(STATUS_FAILED);
            result.addDetail("❌ lta_diplomatic_flag is not 'N': " + diplomaticFlag);
        }
    ...
    private void verifyNoticeStatusAfterUpload(TestStepResult result) {
        ...
        try {
            ...
            // For local flow, we expect ROV -> ENA
    ...
    /**
     * Step 7: Verify final database state
     * Verifies all database tables have been updated correctly for local flow
     *
     * @return TestStepResult containing the result of this step
     */
    private TestStepResult verifyFinalDatabaseState() {
        ...
        result.addDetail("🔍 Verifying final database state for local flow...");
        ...
        try {
            ...
            if (STATUS_SUCCESS.equals(result.getStatus())) {
                result.addDetail("✅ All database verifications passed for local flow");
                result.addDetail("🎉 Local flow test completed successfully!");
            }
        } catch (Exception e) {
            result.setStatus(STATUS_FAILED);
            StringWriter sw = new StringWriter();
    ...
    private void verifyValidOffenceNotice(TestStepResult result) {
        ...
        try {
            ...
            // Verify processing stages for local flow
            String lastProcessingStage = (String) record.get("last_processing_stage");
            String nextProcessingStage = (String) record.get("next_processing_stage");
            ...
            // For local flow after LTA Download, we expect:
            // last_processing_stage = ENA, next_processing_stage = RD1
            if ("ENA".equals(lastProcessingStage)) {
                result.addDetail("✅ last_processing_stage correctly set to ENA");
            } else {
                result.addDetail("❌ last_processing_stage is not ENA: " + lastProcessingStage);
                result.setStatus(STATUS_FAILED);
            }
            ...
            if ("S".equals(vehicleRegType)) {
                result.addDetail("✅ vehicle_registration_type correctly set to S (Local)");
            } else {
                result.addDetail("❌ vehicle_registration_type is not S: " + vehicleRegType);
                result.setStatus(STATUS_FAILED);
            }
    ...
    private void verifyOffenceNoticeDetail(TestStepResult result) {
        ...
        try {
            ...
            if ("N".equals(diplomaticFlag)) {
                result.addDetail("✅ lta_diplomatic_flag correctly set to N");
            } else {
                result.addDetail("❌ lta_diplomatic_flag is not N: " + diplomaticFlag);
                result.setStatus(STATUS_FAILED);
            }
